#!/usr/bin/env python3
"""Seed the imaging_findings Milvus collection from reference JSON data.

Loads finding_seed_data.json, validates each record as an ImagingFinding
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_findings collection.

Usage:
    python scripts/seed_findings.py
    python scripts/seed_findings.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "finding_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_findings collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Findings")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = ImagingFinding(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} ImagingFinding records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} findings into {COLLECTION_NAME}")
    print("=" * 65)
