#!/usr/bin/env python3
"""Seed the imaging_guidelines Milvus collection from reference JSON data.

Loads guideline_seed_data.json, validates each record as a GuidelineRecord
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_guidelines collection.

Usage:
    python scripts/seed_guidelines.py
    python scripts/seed_guidelines.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "guideline_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_guidelines collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Guidelines")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = GuidelineRecord(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} GuidelineRecord records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} guidelines into {COLLECTION_NAME}")
    print("=" * 65)
