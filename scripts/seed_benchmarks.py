#!/usr/bin/env python3
"""Seed the imaging_benchmarks Milvus collection from reference JSON data.

Loads benchmark_seed_data.json, validates each record as a BenchmarkRecord
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_benchmarks collection.

Usage:
    python scripts/seed_benchmarks.py
    python scripts/seed_benchmarks.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "benchmark_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_benchmarks collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Benchmarks")
    print("=" * 65)
    ...
    logger.info(f"Validated {len(records)} BenchmarkRecord records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} benchmarks into {COLLECTION_NAME}")
    print("=" * 65)
