#!/usr/bin/env python3
"""Seed the imaging_datasets Milvus collection from reference JSON data.

Loads dataset_seed_data.json, validates each record as a DatasetRecord
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_datasets collection.

Usage:
    python scripts/seed_datasets.py
    python scripts/seed_datasets.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "dataset_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_datasets collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Datasets")
    print("=" * 65)
    ...
    logger.info(f"Validated {len(records)} DatasetRecord records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} datasets into {COLLECTION_NAME}")
    print("=" * 65)
