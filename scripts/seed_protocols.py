#!/usr/bin/env python3
"""Seed the imaging_protocols Milvus collection from reference JSON data.

Loads protocol_seed_data.json, validates each record as an ImagingProtocol
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_protocols collection.

Usage:
    python scripts/seed_protocols.py
    python scripts/seed_protocols.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "protocol_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_protocols collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Protocols")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = ImagingProtocol(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} ImagingProtocol records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} protocols into {COLLECTION_NAME}")
    print("=" * 65)
