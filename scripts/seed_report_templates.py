#!/usr/bin/env python3
"""Seed the imaging_report_templates Milvus collection from reference JSON data.

Loads report_template_seed_data.json, validates each record as a ReportTemplate
Pydantic model, generates BGE-small-en-v1.5 embeddings, and inserts
into the imaging_report_templates collection.

Usage:
    python scripts/seed_report_templates.py
    python scripts/seed_report_templates.py --host 10.0.0.1 --port 19530
...
SEED_FILE = "report_template_seed_data.json"


def main():
    parser = argparse.ArgumentParser(
        description="Seed imaging_report_templates collection from reference data"
    )
    ...
    print("=" * 65)
    print("  Imaging Intelligence Agent — Seed Report Templates")
    print("=" * 65)
    ...
    for item in raw_data:
        try:
            record = ReportTemplate(**item)
            records.append(record)
        except Exception as e:
            logger.warning(f"Skipping invalid record: {e}")

    logger.info(f"Validated {len(records)} ReportTemplate records")
    ...
    print()
    print("=" * 65)
    print(f"  Seeded {count} report templates into {COLLECTION_NAME}")
    print("=" * 65)
